...
实现数值稳定的平均交叉熵：\(\mathrm{mean}(\log\sum\exp(o)-\,o[\mathrm{target}])\)（`logsumexp` + `gather`）。`test_cross_entropy` 通过。

## Problem: learning_rate_tuning

用讲义 toy SGD（loss=\(\mathrm{mean}(w^2)\)）各跑 10 步（`scripts/lr_tuning.py`，日志 `logs/lr_tuning.log`）：

- `lr=1e1`：loss 从约 22.7 单调降到约 3.0，稳定下降。
- `lr=1e2`：下降很快，数步内接近 0。
- `lr=1e3`：loss 从约 22 爆到 \(10^{18}\) 量级，发散。

## Problem: adamw

实现 `AdamW`（`cs336_basics/optimizer.py`）：`self.state[p]` 存 \(m,v,t\)；按 PDF 做 bias-corrected \(\alpha_t\)、解耦 weight decay \(\theta\leftarrow\theta-\alpha\lambda\theta\)，再更新矩与参数。`test_adamw` 通过。

## Problem: adamw_accounting

全用 fp32（4 bytes/元素）。记 \(B=\mathrm{batch\_size}\)，\(T=\mathrm{context\_length}\)，\(L=\mathrm{num\_layers}\)，\(D=\mathrm{d\_model}\)，\(H=\mathrm{num\_heads}\)，\(V=\mathrm{vocab\_size}\)，\(d_{ff}=\frac{8}{3}D\)。

### (a)

参数量
\[
P=2VD+L(4D^{2}+3Dd_{ff}+2D)+D=2VD+L(12D^{2}+2D)+D.
\]
...
| parameters | \(P\) | \(4P\) |
| gradients | \(P\) | \(4P\) |
| optimizer state（\(m,v\)） | \(2P\) | \(8P\) |
| activations | 见下 | \(4A\) |

激活（仅按题面列出的张量，每层）：RMSNorm×2（\(2BTD\)）；QKV（\(3BTD\)）、\(QK^{\top}\) 与 softmax（各 \(BHT^{2}\)）、Attn\(V\) 与 output（各 \(BTD\)）；SwiGLU 的 \(W_1,W_3,\mathrm{SiLU}\), product（各 \(BTd_{ff}\)）与 \(W_2\)（\(BTD\)）。另加 final RMSNorm（\(BTD\)）、`lm_head` logits（\(BTV\)）、CE 相关（再计 \(BTV\)）。

\[
A=L\big(8BTD+2BHT^{2}+4BTd_{ff}\big)+BTD+2BTV,
\]
\(d_{ff}=\frac{8}{3}D\) 时每层可写成 \(\frac{56}{3}BTD+2BHT^{2}\)。
...
GPT-2 XL（\(V{=}50257,T{=}1024,L{=}48,D{=}1600,H{=}25\)）：\(P=1{,}635{,}537{,}600\)，

\[
\mathrm{peak}\approx 16.36\,\mathrm{GB}\cdot B + 26.17\,\mathrm{GB}.
\]
...
AdamW 每步对每个参数做衰减、更新 \(m/v\)、再写回 \(\theta\)，量级约 **\(12P\) FLOPs**（与前向的 \(O(BTD^{2}L)\) 相比可忽略）。

### (d)

单序列前向约 \(F\approx 3.507\times 10^{12}\) FLOPs；一步按 fwd+2×bwd 计 \(3BF\)。400K step、\(B{=}1024\)、H100 峰值 \(495\,\mathrm{TFLOP/s}\)、50% MFU：

\[
t=\frac{4\times 10^{5}\cdot 3\cdot 1024\cdot F}{0.5\cdot 495\times 10^{12}}\approx 1.74\times 10^{7}\,\mathrm{s}\approx \mathbf{4836}\,\mathrm{hours}.
\]

## Problem: learning_rate_schedule

实现 cosine schedule with warmup（`get_lr_cosine_schedule`）：\(t<T_w\) 线性升到 \(\alpha_{\max}\)；\(T_w\le t\le T_c\) 余弦降到 \(\alpha_{\min}\)；之后保持 \(\alpha_{\min}\)。`test_get_lr_cosine_schedule` 通过。

## Problem: gradient_clipping

实现全局 \(\ell_2\) 梯度裁剪：对所有非 `None` 的 `.grad` 算总范数，若超过 `max_l2_norm` 则统一乘 \(\frac{\mathrm{max\_l2\_norm}}{\|g\|_2+\varepsilon}\)（\(\varepsilon=10^{-6}\)），原地修改。`test_gradient_clipping` 通过。
